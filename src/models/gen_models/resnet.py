...
class ResNetGenerator(chainer.Chain):
    def __init__(self, ch=64, dim_z=128, bottom_width=4, activation=F.relu, distribution="normal"):
        ...
        with self.init_scope():
            ...
            self.block2 = Block(ch * 16, ch * 16, activation=activation, upsample=True)
            self.block3 = Block(ch * 16, ch * 8, activation=activation, upsample=True)
            self.block4 = Block(ch * 8, ch * 4, activation=activation, upsample=True)
            self.block5 = Block(ch * 4, ch * 2, activation=activation, upsample=True)
            self.block6 = Block(ch * 2, ch, activation=activation, upsample=True)
